...
/*
  ...
  TEST CASE 246: nanoseconds elapsed: 1494661600
		...
		while (!Tasks_Thread.empty() && !QED && !bTimeoutFlag)
		{
			...
			if (TentativeProofFound_Flag)
			{
				...
				*/
				...
				std::for_each (std::execution::par_unseq, Axioms_UInt64Vec.begin(), Axioms_UInt64Vec.end(), [&] (const auto& Axiom)
				{
					...
					//std::async(std::launch::async, [&] () {
						if (theoremLHS % AxiomLHS == 0)
						{
							...
							{
								std::unique_lock<std::shared_mutex> taskslock(tasksMutex);
